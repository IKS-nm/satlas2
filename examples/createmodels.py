...
class ExpSine(satlas2.Model):
    def __init__(self, A, lamda, omega, name="ExpSine", prefunc=None):
        ...
        self.params = {
            "amplitude": satlas2.Parameter(
                value=A, min=0, max=np.inf, vary=True
            ),
            "lambda": satlas2.Parameter(
                value=lamda, min=0, max=np.inf, vary=True
            ),
            "omega": satlas2.Parameter(
                value=omega, min=0, max=np.inf, vary=True
            ),
        }
