...
hfs = satlas2.HFS(
    spin, J, A, B, C, df=centroid, fwhmg=FWHMG, fwhml=FWHML, scale=scale
)
...
ax_intervaldata.fill_between(
    x, lowerd2, upperd2, color=lc.get_color(), alpha=0.3, step="mid"
)
ax_intervaldata.fill_between(
    x, lowerd3, upperd3, color=lc.get_color(), alpha=0.3, step="mid"
)
...
ax_intervalspectrum.fill_between(
    plot_x,
    lowerm1,
    upperm1,
    color=lc.get_color(),
...
ax_intervalspectrum.fill_between(
    plot_x, lowerm2, upperm2, color=lc.get_color(), alpha=0.3, step="mid"
)
ax_intervalspectrum.fill_between(
    plot_x, lowerm3, upperm3, color=lc.get_color(), alpha=0.3, step="mid"
)
