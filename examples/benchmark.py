...
for j in range(1, 11):
    ...
    for i in range(j):
        ...
        models.append(hfs1)
        datasource = satlas2.Source(
            x, y, yerr=modifiedSqrt, name="Scan{}".format(i + 1)
        )
...
for i, (name, datasource) in enumerate(f.sources):
    ...
    smooth_left = np.arange(
        datasource.x[left].min(), datasource.x[left].max(), 5.0
    )
    smooth_right = np.arange(
        datasource.x[right].min(), datasource.x[right].max(), 5.0
    )
    ax1.plot(
        datasource.x[left],
        datasource.y[left],
...
for i, (name, datasource) in enumerate(f.sources):
    ...
    axes[i][1].plot(
        smooth_right, datasource.evaluate(smooth_right), label="Fit"
    )
a1.legend(loc=0)
