...
start = time.time()
succes, message = satlas2.chisquare_fit(
    hfs, x, y, modifiedSqrt(y), show_correl=False
)
...
dt2 = stop - start
print(
    "SATLAS2: {:.3} s, {:.0f} function evaluations".format(
        dt1, hfs.fitter.result.nfev
    )
)
...
summodel = satlas2.SumModel(
    [hfs1, hfs2], {"values": [bkg1, bkg2], "bounds": [0]}
)

print("Fitting 1 dataset with chisquare (Pearson, satlas2)...")
start = time.time()
succes, message = satlas2.chisquare_fit(
    summodel, x, y, modifiedSqrt(y), show_correl=False
)
print(succes, message)
...
print(
    "SATLAS2: {:.3} s, {:.0f} function evaluations".format(
        dt1, summodel.fitter.result.nfev
    )
)
